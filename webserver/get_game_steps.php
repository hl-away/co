...
if( strlen($game_id) > 0 && strlen($last_step) > 0) {
    ...
    if( mysql_num_rows($r) > 0) {
        for($i = 0; $row = mysql_fetch_assoc($r); $i++) {
            ...
            $user_id = $row['USER_ID'];
            $object_data = "";
            if( $type == "add_city" || $type == "add_new_city" ) {
                ...
                $r_c = mysql_query("SELECT ID, LOGIN, SCORE FROM USER WHERE ID = $object_id");
                if( mysql_num_rows($r_c) > 0 ) {
                    $object_data = "i=" .mysql_result($r_c,0,0) ."#l=" .mysql_result($r_c,0,1) ."#s=" .mysql_result($r_c,0,2);
                }
            }
            echo "s:" .$step ."&t:" .$type ."&u:" .$user_id ."&o:" .$object_data;
        }
    } else {
        echo "NULL";
    }
